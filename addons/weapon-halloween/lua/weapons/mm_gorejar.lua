...
function SWEP:Initialize()
	...
	self.Owner:SetNWFloat("mm_gorejar_recharge", 0)
end
...
function SWEP:Think()
	...
	if self.Weapon:Clip1() < 1 then
		...
		return
	elseif self.Owner:GetNWFloat("mm_gorejar_recharge") != 0 then
	...
	if !self.Owner:KeyDown(IN_ATTACK) && self:GetPrimed() == true && self:GetMM_PrimeTime() > 175 then
		...
		if SERVER then
			...
			local grenade = ents.Create("ent_gorejar")
		...
		self.Owner:SetNWFloat("mm_gorejar_recharge", CurTime()+20)
		if SERVER then
			self.Owner:SelectWeapon("mm_gorejar")
