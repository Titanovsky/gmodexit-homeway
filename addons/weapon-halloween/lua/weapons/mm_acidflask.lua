...
function SWEP:Think()
	...
	if !self.Owner:KeyDown(IN_ATTACK) && self:GetPrimed() == true && self:GetMM_PrimeTime() > 175 then
		self:SetPrimed(false)
...
function SWEP:OnDrop()
	if self.HandDropped == false && self.Weapon:Clip1() > 0 then
		local grenade = ents.Create("ent_acidflask")
 		grenade:SetPos(self:GetPos())
		grenade:SetAngles(Angle(0,0,0))
		grenade:SetOwner(self.MyOwner)
		grenade:Spawn()
		grenade:Activate()
