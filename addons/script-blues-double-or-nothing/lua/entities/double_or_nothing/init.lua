...
function ENT:Think()
	...
	if self.user ~= nil and self.timeSinceLastUsed + 10 - CurTime() < 0 then
		--Times up
		self.user = nil
		self:SetUserName("Nobody")
	end
...
function ENT:Use(act, call)
	hook.Call( '[Ambi.Homeway.CasinoFourDragon]', nil, call, self )
...
function ENT:Think()
	if self.user ~= nil then
		...
		if CurTime() - self.user.bdonTimeSinceLastUse >= 10 then
			self:UnlinkUser()
		end
	end
end
...
timer.Create("bdon:cleanUpUsedMachines", 1, 0, function()
	for k ,v in pairs(player.GetAll()) do
		if v.bdonTimeSinceLastUse ~= nil and v.bdonLastUsedSlot ~= nil and CurTime() - v.bdonTimeSinceLastUse > 10 then
