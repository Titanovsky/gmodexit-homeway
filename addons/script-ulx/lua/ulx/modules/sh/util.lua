...
function ulx.kick( calling_ply, target_ply, reason )
	...
	hook.Call( '[Ambi.ULX.kick]', nil, calling_ply, target_ply, reason )
end
...
function ulx.ban( calling_ply, target_ply, minutes, reason )
	...
	hook.Call( '[Ambi.ULX.ban]', nil, calling_ply, target_ply, minutes, reason )
end
...
function ulx.banid( calling_ply, steamid, minutes, reason )
	...
	hook.Call( '[Ambi.ULX.banid]', nil, calling_ply, steamid, minutes, reason )
end
...
function ulx.unban( calling_ply, steamid )
	...
	hook.Call( '[Ambi.ULX.unban]', nil, calling_ply, steamid )
end
...
function ulx.spectate( calling_ply, target_ply )
	...
	calling_ply.nw_Spectating = true
...
local function playerPickup( ply, ent )
	...
	if ent:GetClass() == "player" and ULib.isSandbox() and access and not ent.NoNoclip and not ent.frozen and ply:GetInfoNum( cl_cvar_pickup, 1 ) == 1 then
		...
		if SERVER and ent.nw_Nocliping then 
			ent.nw_Nocliping = false
			ULib.invisible( ent, false ) 
		end
		return true
	end
end
