...
------------------------------ Jail ------------------------------
function ulx.jail( calling_ply, eTarget, nMinutes, sReason )
	eTarget:Jail( nMinutes, sReason )

	ulx.fancyLogAdmin( calling_ply, "#A заджайлил #T на #i минут. Причина: #s", eTarget, nMinutes, sReason )

	hook.Call( '[Ambi.ULX.jail]', nil, calling_ply, eTarget, nMinutes, sReason )
end
local jail = ulx.command( CATEGORY_NAME, "ulx jail", ulx.jail, "!jail" )
jail:addParam{ type=ULib.cmds.PlayerArg }
jail:addParam{ type=ULib.cmds.NumArg, min=0, default=0, max = 500, hint="Минуты", ULib.cmds.round, ULib.cmds.optional }
jail:addParam{ type=ULib.cmds.StringArg, hint="Причина" }
jail:defaultAccess( ULib.ACCESS_ADMIN )
jail:help( "Отправить в джайл зону" )

function ulx.unjail( calling_ply, eTarget, sReason )
	eTarget:UnJail()

	ulx.fancyLogAdmin( calling_ply, "#A убрал из джайла #T. Причина: #s", eTarget, sReason )

	hook.Call( '[Ambi.ULX.unjail]', nil, calling_ply, eTarget, sReason )
end
local unjail = ulx.command( CATEGORY_NAME, "ulx unjail", ulx.unjail, "!unjail" )
unjail:addParam{ type=ULib.cmds.PlayerArg }
unjail:addParam{ type=ULib.cmds.StringArg, hint="Причина" }
unjail:defaultAccess( ULib.ACCESS_ADMIN )
unjail:help( "Убрать из джайла" )
