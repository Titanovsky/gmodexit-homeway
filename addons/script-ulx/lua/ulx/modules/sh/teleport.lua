...
function ulx.bring( calling_ply, target_plys )
  ...
  if #teleportable_plys > 0 then
	...
	if #affected_plys > 0 then
  	ulx.fancyLogAdmin( calling_ply, true, "#A телепортировал #T", affected_plys )
	end
end
...
function ulx.goto( calling_ply, target_ply )
	...
	ulx.fancyLogAdmin( calling_ply, true, "#A тпхнулся к #T", target_ply )
end
...
function ulx.send( calling_ply, target_from, target_to )
	...
	ulx.fancyLogAdmin( calling_ply, true, "#A телепортивал #T к #T", target_from, target_to )
end
...
function ulx.teleport( calling_ply, target_ply )
	...
	if target_ply ~= calling_ply then
		ulx.fancyLogAdmin( calling_ply, true, "#A телепортировал #T", target_ply ) -- We don't want to log otherwise
	end
end
...
function ulx.retrn( calling_ply, target_ply )
	...
	if target_ply:IsJail() then target_ply:Spawn() end

	ulx.fancyLogAdmin( calling_ply, true, "#A вернул #T", target_ply )
end
