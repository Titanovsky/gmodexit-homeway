...
function ulx.adduser( calling_ply, target_ply, group_name, nHours )
	...
	target_ply:SetRank( group_name, nHours )

	ulx.fancyLogAdmin( calling_ply, "#A задал для #T ранг #s на #i часов", target_ply, group_name, nHours )

	hook.Call( '[Ambi.ULX.adduser]', nil, calling_ply, target_ply, group_name, nHours )
end
...
adduser:addParam{ type=ULib.cmds.NumArg, min=-1, default=-1, hint="Сколько часов (-1 бесконечно)", ULib.cmds.optional, ULib.cmds.round }
adduser:defaultAccess( ULib.ACCESS_SUPERADMIN )
adduser:help( "Add a user to specified group." )

function ulx.adduserid( calling_ply, id, group_name, nHours )
	...
	Ambi.Homeway.SetRank( id, group_name, nHours )

	if ULib.ucl.users[ id ] and ULib.ucl.users[ id ].name then
		ulx.fancyLogAdmin( calling_ply, "#A задал для #s ранг #s на #i часов", ULib.ucl.users[ id ].name, group_name, nHours )
	else
		ulx.fancyLogAdmin( calling_ply, "#A задал для #s ранг #s на #i часов", id, group_name, nHours )
	end

	hook.Call( '[Ambi.ULX.adduserid]', nil, calling_ply, id, group_name, nHours )
end
...
adduserid:addParam{ type=ULib.cmds.NumArg, min=-1, default=-1, hint="Сколько часов (-1 бесконечно)", ULib.cmds.optional, ULib.cmds.round }
...
function ulx.removeuser( calling_ply, target_ply )
	...
	target_ply:RemoveRank()

	ulx.fancyLogAdmin( calling_ply, "#A удалил ранг у #T", target_ply )

	hook.Call( '[Ambi.ULX.removeuser]', nil, calling_ply, target_ply )
end
...
function ulx.removeuserid( calling_ply, id )
	...
	Ambi.Homeway.RemoveRank( id )
	ULib.ucl.removeUser( id )

	if name then
		ulx.fancyLogAdmin( calling_ply, "#A удалил ранг у #s", name )
	else
		ulx.fancyLogAdmin( calling_ply, "#A удалил ранг у #s", id )
	end

	hook.Call( '[Ambi.ULX.removeuserid]', nil, calling_ply, id )
end
