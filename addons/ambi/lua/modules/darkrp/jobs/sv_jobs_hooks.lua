...
hook.Add( 'PlayerInitialSpawn', 'Ambi.DarkRP.SetFirstJob', function( ePly )
    ePly.darkrp_initial = true

    -- timer.Simple( 1, function() --todo broken
...
hook.Add( 'PlayerSpawn', 'Ambi.DarkRP.SetJobStats', function( ePly )
    timer.Simple( 0, function()
        ...
        if ePly.darkrp_initial then
            local class = Ambi.DarkRP.Config.jobs_class

            local job = Ambi.DarkRP.GetJob( class )
            if not job then ePly:Kick( '[DarkRP] Не создана дефолтная профессия: '..class ) return end

            if not ePly:IsBot() then
                if Ambi.DarkRP.Config.jobs_permanent_enable then 
                    local permaclass = SQL.Select( DB, 'Class', 'SteamID', ePly:SteamID() )

                    if permaclass and Ambi.DarkRP.GetJob( permaclass ) then class = permaclass end 
                else
                    class = Ambi.DarkRP.Config.jobs_class
                end
            end

            ePly:SetJob( class, true )

            ePly.darkrp_initial = nil

            hook.Call( '[Ambi.DarkRP.SetFirstJob]', nil, ePly )
        end

        if not ePly:IsArrested() then
...
hook.Add( 'PlayerDisconnected', 'Ambi.DarkRP.JobCustomField', function( ePly )
    local job = Ambi.DarkRP.GetJob( ePly:GetJob() )
    if not job then return end

    local Action = job.PlayerDisconnected
    if Action then
        return Action( ePly )
    end
end )
