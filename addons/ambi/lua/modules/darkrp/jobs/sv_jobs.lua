...
function PLAYER:SetJob( sClass, bForce, bRespawn )
    ...
    if not bForce then 
        ...
        if self:GetBlockJob( sClass ) then self:ChatSend( '~ERROR~ • ~W~ Работа ~ERROR~ '..job.name..' ~W~ заблокирована для Вас'..( self:GetBlockJobTime( sClass ) and ' на ~ERROR~ '..tostring( math.Round( self:GetBlockJobTime( sClass ) ) )..' ~W~ секунд' or '' ) )  return false end
    ...
    if Ambi.DarkRP.Config.jobs_respawn and ( bRespawn != false ) then 
        ...
        Ambi.DarkRP.GiveBaseWeapons( self )
    else
        Ambi.DarkRP.GiveBaseStats( self )
...
function PLAYER:BlockJob( sClass, nTime )
    ...
    if nTime and ( nTime > 0 ) then
        timer.Create( 'Ambi.DarkRP.BlockJob:'..sClass..':'..self:SteamID(), nTime, 1, function()
...
function PLAYER:GetBlockJobTime( sClass )
    if not sClass or ( sClass == Ambi.DarkRP.Config.jobs_class ) then return end
    ...
    local sid = self:SteamID()
    if not Ambi.DarkRP.players_block_jobs[ sid ] then return end
    if not Ambi.DarkRP.players_block_jobs[ sid ][ sClass ] then return end
    if not timer.Exists( 'Ambi.DarkRP.BlockJob:'..sClass..':'..self:SteamID() ) then return end

    return timer.TimeLeft( 'Ambi.DarkRP.BlockJob:'..sClass..':'..self:SteamID() )
end
...
function PLAYER:SetJobFeatures()
    ...
    if self.job_model then
        local model = job.models[ 1 ]
        for _, mdl in ipairs( job.models ) do
            if ( mdl == self.job_model ) then model = self.job_model break end
        end
...
function PLAYER:StripJobWeapons()
    for _, wep in ipairs( self:GetWeapons() ) do
        if not wep.job_weapon then continue end

        local ammo1 = wep:GetPrimaryAmmoType()
        local ammo2 = wep:GetSecondaryAmmoType()
        local ammo1_count = self:GetAmmoCount( ammo1 )
        local ammo2_count = self:GetAmmoCount( ammo2 )
        self:RemoveAmmo( ammo1_count, ammo1 )
        self:RemoveAmmo( ammo2_count, ammo2 )
...
net.Receive( 'ambi_darkrp_demote_player', function( _, ePly )
    ...
    if not demoter:CanDemoteJob( ePly, '' ) then return end

    local job = demoter:GetJobTable()
    if not job then return end
    if ( job.demote == false ) then ePly:ChatSend( C.ERROR, '• ', C.ABS_WHITE, 'Игроков с работой '..job.name..' нельзя уволить' ) return end
    if ( demoter:GetJob() == Ambi.DarkRP.Config.jobs_class ) then ePly:ChatSend( C.ERROR, '• ', C.ABS_WHITE, 'У игрока дефолтная работа' ) return end
