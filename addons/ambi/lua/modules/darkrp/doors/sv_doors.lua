...
function Ambi.DarkRP.SetDoorBlockOwn( eDoor ) -- Запретить владеть вообще кому-либо дверью
    ...
    SQL.Get( DB, 'Door', 'Door', index, function()
        ...
        SQL.Insert( DB, 'Door, Map, IsBlock, Category', '%i, %s, %s, %s', index, game.GetMap(), 'true', '' )
    end )
...
function Ambi.DarkRP.RemoveDoorOwners( eDoor )
    ...
    for i, ply in ipairs( eDoor.owners or {} ) do
        ply.doors[ eDoor ] = nil
    end 
...
local function BuyDoor( ePly )
    ...
    if Ambi.DarkRP.doors_categories[ door.nw_Category or '' ] then return end
