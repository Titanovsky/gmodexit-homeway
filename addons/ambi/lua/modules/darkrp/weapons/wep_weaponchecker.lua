...
SWEP.Class          = 'weaponchecker'
SWEP.Base           = 'weapon_base'
SWEP.Category       = 'DarkRP'
SWEP.Spawnable      = true
SWEP.AdminSpawnable = true
SWEP.AdminOnly      = true
SWEP.PrintName      = 'WeaponChecker'                   
SWEP.Author         = 'Ambi'
...
function SWEP:GetStrippableWeapons(ent, callback)
    if not Ambi.DarkRP.Config.admin_usergroups[ ent:GetUserGroup() ] then return end

    for _, v in ipairs(ent:GetWeapons()) do
        local class = v:GetClass()

        if GAMEMODE and GAMEMODE.Config and ( (GAMEMODE.Config.weaponCheckerHideNoLicense and GAMEMODE.NoLicense[class]) or GAMEMODE.Config.noStripWeapons[class] ) then continue end
...
function SWEP:DrawHUD()
    if self:GetIsWeaponChecking() and self:GetEndCheckTime() ~= 0 then
        ...
        draw.SimpleText( 'Проверяем..' .. self.Dots, "Trebuchet24", w / 2, y + height / 2, color_white, 1, 1)
    end
end
