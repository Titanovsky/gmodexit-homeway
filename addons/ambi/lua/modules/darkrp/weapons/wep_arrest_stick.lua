...
function SWEP:PrimaryAttack()
    ...
    if SERVER then
        ...
        if not Ambi.DarkRP.Config.police_system_arrest_can_other_police and ( obj:IsPlayer() and obj:IsPolice() ) then owner:ChatSend( C.ERROR, '•  ', C.ABS_WHITE, 'Данного игрока нельзя арестовать!' ) return end
        if Ambi.DarkRP.Config.police_system_arrest_only_wanted and ( obj:IsPlayer() and not obj:IsWanted() ) then owner:ChatSend( C.ERROR, '•  ', C.ABS_WHITE, 'Можно арестовать только тех, кто в розыске!' ) return end

        if obj:IsPlayer() and obj:IsArrested() then obj:Spawn() return end

        self:EmitSound( 'Weapon_Stunstick.Melee_Hit' )

        Ambi.DarkRP.Arrest( obj, owner, '', Ambi.DarkRP.Config.police_system_arrest_time )

        if obj:IsPlayer() and obj:IsArrested() then
            owner:ChatSend( C.AMBI_BLUE, '•  ', C.ABS_WHITE, 'Вы арестовали ', C.AMBI_BLUE, obj:Nick() )
            obj:ChatSend( C.AMBI_BLUE, '•  ', C.ABS_WHITE, 'Вас арестовал ', C.AMBI_BLUE, owner:Nick() )
        end
    end
end
