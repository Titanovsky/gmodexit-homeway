...
function PLAYER:BuyShopItem( sClass, bForce )
    ...
    if not bForce then
        ...
        if item.CanBuy and ( item.CanBuy( self ) == false ) then return false end
...
function Ambi.DarkRP.SpawnShopEntity( sClass, ePly, vPos, aAng ) 
    ...
    if item.remove_after_set_job then ent.remove_after_set_job = true end
...
hook.Add( 'PlayerDisconnected', 'Ambi.DarkRP.SetShopEntities', function( ePly ) 
    ...
    for ent, _ in pairs( items ) do
        ent.nw_BuyerName = ePly:Name()..' [Вышел]'
    end
end )

hook.Add( '[Ambi.DarkRP.SetJob]', 'Ambi.DarkRP.SetShopEntities', function( ePly ) 
    local items = Ambi.DarkRP.players_shop_items[ ePly:SteamID() ]
    if not items then return end

    for ent, class in pairs( items ) do
        if Ambi.DarkRP.GetShopItem( class ).remove_after_set_job then ent:Remove() end
    end
end )
