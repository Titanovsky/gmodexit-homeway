...
function Ambi.DarkRP.GiveBaseStats( ePly )
...
function Ambi.DarkRP.GiveBaseWeapons( ePly )
    timer.Simple( 0, function()
        if not IsValid( ePly ) then return end

        for _, class in ipairs( Ambi.DarkRP.Config.player_base_weapons ) do
            ePly:Give( class )
            ePly:GetWeapon( class ).cannot_drop = true
        end

        print( '[DarkRP] Player '..tostring( ePly )..' Gave Base Weapons ' )
    end )
end
...
hook.Add( 'PlayerLoadout', 'Ambi.DarkRP.GiveBaseWeapons', function( ePly )
    ...
    timer.Simple( 0.1, function()
        Ambi.DarkRP.GiveBaseWeapons( ePly )
    end )
...
hook.Add( 'PlayerSpawn', 'Ambi.DarkRP.GiveBaseStats', function( ePly ) 
    timer.Simple( 0, function()
        if IsValid( ePly ) then Ambi.DarkRP.GiveBaseStats( ePly ) end
    end )
end )
