...
if CLIENT then
    concommand.Add( 'ambi_darkrp_'..Ambi.DarkRP.Config.money_drop_command, function( ePly, _, tArgs )
        ...
        if ( amount < Ambi.DarkRP.Config.money_drop_min ) then return end
    ...
    net.Receive( msg, function( _, ePly )
        ...
        if ( amount < Ambi.DarkRP.Config.money_drop_min ) then ePly:Kick( '[DarkRP] Попытка неправильного dropmoney (Меньше минимума)' ) return end
...
Add( Ambi.DarkRP.Config.money_drop_command, TYPE, Ambi.DarkRP.Config.money_drop_description, 0.15, function( ePly, tArgs ) 
    ...
    amount = tonumber( amount )
    if ( amount < Ambi.DarkRP.Config.money_drop_min ) then ePly:ChatSend( '~R~ • ~W~ Минимальная сумаа для дропа ~R~ '..Ambi.DarkRP.Config.money_drop_min ) return end
