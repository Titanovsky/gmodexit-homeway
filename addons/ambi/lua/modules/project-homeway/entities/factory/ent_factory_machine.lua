...
function ENT:Use( ePly )
    ...
    if ePly:IsPolice() then ePly:Notify( 'Вам нельзя здесь работать!', 4, NOTIFY_ERROR ) return end
    ...
    if ( ePly.factory_count >= Ambi.Homeway.Config.factory_work_delay_count ) then
        timer.Create( 'Ambi.Homeway.FactoryRelax:'..ePly:SteamID(), Ambi.Homeway.Config.factory_work_delay, 1, function() 
            if IsValid( ePly ) then ePly.factory_count = 0 end
        end )
    end
end
